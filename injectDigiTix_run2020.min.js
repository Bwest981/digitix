...
async function purchaseTicket(eventId, ticketTierId, ticketCost,that){
  ...
  await promisify(cb => tokenContract.allowance(userAccount,contractAddress,cb))
    .then(function (result){
      console.log(result);
      if(result > 0){
        ticketContract.purchaseTicket(eventId,ticketTierId,purchaseTicketQty, purchaseTicketPayWithTokens,{from: web3.eth.accounts[0], gas:400000, value:0 , gasPrice:60000000000})
        .catch(console.error)
        .once('transactionHash', function(hash){
          console.log("Hash: " +hash );
        })
